...
impl Tool for EditFileTool {
    ...
    async fn call(
        ...
        let old_content = require_str(Self::TOOL_NAME, &input, "old_content")
            .map_err(AgtrsError::from)?;
        let new_content = require_str(Self::TOOL_NAME, &input, "new_content")
            .map_err(AgtrsError::from)?;
        ...
        editor.commit().await.map_err(|e| AgtrsError::ToolCallFailed {
        ...
        tracing::info!(
            path,
            lines_delta = receipt.lines_delta,
            "edit_file"
        );
